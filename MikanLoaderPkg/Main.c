...
const CHAR16* GetMemoryTypeUnicode(EFI_MEMORY_TYPE type) {
    switch (type) {
        case EfiReservedMemoryType: return L"EfiReservedMemoeryType";
...
EFI_STATUS OpenRootDir(EFI_HANDLE image_handle, EFI_FILE_PROTOCOL** root) {
    ...
    status = gBS->OpenProtocol(
        image_handle, &gEfiLoadedImageProtocolGuid, (VOID**)&loaded_image, image_handle,
...
EFI_STATUS OpenGOP(EFI_HANDLE image_handle, EFI_GRAPHICS_OUTPUT_PROTOCOL** gop) {
    ...
    status = gBS->OpenProtocol(gop_handles[0], &gEfiGraphicsOutputProtocolGuid, (VOID**)gop,
        image_handle, NULL, EFI_OPEN_PROTOCOL_BY_HANDLE_PROTOCOL);
    if (EFI_ERROR(status)) {
        return status;
    }
...
// https://dox.ipxe.org/GraphicsOutput_8h.html
...
void CalcLoadAddressRange(Elf64_Ehdr* ehdr, UINT64* first, UINT64* last) {
    ...
    *last = 0;
    ...
    Print(L"count(PT_LOAD) / number of program header = (%d / %d)\n", load_cnt, ehdr->e_phnum);
}
    ...
    EFI_SYSTEM_TABLE *system_table)
{
    ...
    if (EFI_ERROR(status)) {
        Print(L"failed to read the kernel file: %r", status);
        Halt();
    }
    ...
    // The size of a page is fixed in ELF
    const int SIZE_OF_PAGE = 0x1000;
    UINTN num_pages = (kernel_file_size + SIZE_OF_PAGE - 1) / SIZE_OF_PAGE;
    ...
    // The offset for 64bit system defined in ELF
    const int OFFSET_OF_ENTRYPOINT_ADDRESS = 24;
    UINT64 entry_addr = *(UINT64*)(kernel_first_addr + OFFSET_OF_ENTRYPOINT_ADDRESS);
