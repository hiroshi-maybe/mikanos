...
Error FrameBuffer::Initialize(const FrameBufferConfig& config) {
    ...
    if (bytes_per_pixel <= 0) return MAKE_ERROR(Error::kUnknownPixelFormat);
...
Error FrameBuffer::Copy(Vector2D<int> pos, const FrameBuffer& src) {
    ...
    const Vector2D<int> copy_start = ElementMax(pos, {0, 0});
    const Vector2D<int> copy_end = ElementMin(pos + src_size, dst_size);

    uint8_t* dst_buf = FrameAddrAt(copy_start, config_);
    const uint8_t* src_buf = FrameAddrAt({0, 0}, src.config_);

    for (int y = copy_start.y; y < copy_end.y; ++y) {
        memcpy(dst_buf, src_buf, bytes_per_pixel * (copy_end.x - copy_start.x));
