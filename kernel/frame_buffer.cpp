...
int calc_bytes_per_pixel(int bits_per_pixel) {
    return (bits_per_pixel + 7) / 8;
}

int calc_buf_size(int bits_per_pixel, int width, int height, int x_offset) {
    return calc_bytes_per_pixel(bits_per_pixel) * width * height + x_offset;
}
...
Error FrameBuffer::Initialize(const FrameBufferConfig& config) {
    ...
    if (bits_per_pixel <= 0) return MAKE_ERROR(Error::kUnknownPixelFormat);

    if (config_.frame_buffer) {
        buffer_.resize(0);
    } else {
        auto buf_size = calc_buf_size(
            bits_per_pixel, config_.horizontal_resolution, config_.vertical_resolution, 0);
...
Error FrameBuffer::Copy(Vector2D<int> pos, const FrameBuffer& src) {
    ...
    const auto bytes_per_pixel = calc_bytes_per_pixel(bits_per_pixel);
    const auto bytes_per_copy_line = bytes_per_pixel * (copy_end_dst_x - copy_start_dst_x);
    uint8_t* dst_buf = config_.frame_buffer + calc_buf_size(bits_per_pixel,
